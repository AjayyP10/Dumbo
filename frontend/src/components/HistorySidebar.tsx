...
const HistoryEntry = memo(
  ({ item, onSelect, toggle }: { item: HistoryItem; onSelect: (i: HistoryItem) => void; toggle: () => void }) => (
    <button
      onClick={() => {
        onSelect(item);
        toggle();
    ...
    >
      <p className="font-medium text-gray-900 dark:text-gray-100 truncate mb-1 text-base" title={item.text}>
        {item.text}
      </p>
      <p className="text-sm text-gray-500 dark:text-gray-400 truncate mb-2" title={item.translation}>
        {item.translation}
      </p>
...
const HistorySidebar = memo(function HistorySidebar({ items, onSelect, onClear, isOpen, toggle }: Props) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-0 right-0 h-full w-80 bg-white dark:bg-gray-900 shadow-2xl border-l border-gray-200 dark:border-gray-700 transform transition-transform duration-300 ease-in-out z-50 flex flex-col ${isOpen ? "translate-x-0" : "translate-x-full"
          }`}
        ...
        <div className="flex-1 overflow-y-auto">
          {items.length === 0 ? (
            ...
            <div className="divide-y divide-gray-100 dark:divide-gray-700">
              {items.map((item) => (
                <HistoryEntry key={item.id} item={item} onSelect={onSelect} toggle={toggle} />
              ))}
            </div>
          )}
        </div>
